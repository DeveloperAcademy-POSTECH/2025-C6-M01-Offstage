...
struct LegacyOnboardingView: View {
    ...
    private let startPage = LegacyOnboardingPage(
    ...
    private let pages: [LegacyOnboardingPage] = [
        .init(
        ...
        VStack(spacing: 0) {
            Group {
                if currentPage == 0 {
                    ...
                    // 권한 화면
                    LegacyOnboardingPermissionsView(
                        previousButtonTapped: {
                            if currentPage > 0 {
                                currentPage -= 1
                            }
...
#Preview {
    LegacyRouterView(router: LegacyRouter<LegacyAppRoute>(root: .onboarding))
}
