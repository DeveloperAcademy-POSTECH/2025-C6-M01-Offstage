...
struct LegacyBusStationView: View {
    @EnvironmentObject private var router: LegacyRouter<LegacyAppRoute>
    ...
    var body: some View {
        ZStack(alignment: .bottomTrailing) {
            VStack(spacing: 16) {
                ...
                Button {
                    let destination = LegacyAppRoute
                        .busVision(routeToDetect: favoritedRoutesInThisStation.map(\.routeNo))
    ...
    private var content: some View {
        ...
        case let .success(routes):
            if routes.isEmpty {
                ...
                LegacyBusStationListSubView(routes: routes, viewInput: viewModel.input)
                    .padding(.horizontal, 16)
            }
