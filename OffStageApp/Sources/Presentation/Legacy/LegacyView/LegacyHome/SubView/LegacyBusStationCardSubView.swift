...
struct LegacyBusStationCardSubView: View {
    ...
    var body: some View {
        VStack {
            ...
            LegacyBusRouteListSubView(busArrivals: viewModel.busArrivals)

            Button {
                let destination = LegacyAppRoute.busVision(routeToDetect: favorites.map(\.routeNo))
